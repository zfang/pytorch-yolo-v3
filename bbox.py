...
def bbox_iou(box1, box2):
    ...
    def get_zeros(shape):
        return torch.zeros(shape).type(box1.type())

    # Intersection area
    inter_area = torch.max(inter_rect_x2 - inter_rect_x1 + 1, get_zeros(inter_rect_x2.shape)) * \
                 torch.max(inter_rect_y2 - inter_rect_y1 + 1, get_zeros(inter_rect_y2.shape))
